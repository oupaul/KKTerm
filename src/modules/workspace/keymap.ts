...
const FIXED_TERMINAL_SHORTCUT_ALIASES: ReadonlyArray<{
  ...
  // macOS convention: Cmd+C copies the terminal selection. Kept as a fixed
  // alias (not the primary configurable binding, which stays Ctrl+Shift+C for
  // cross-platform consistency) so mac users get native copy without the
  // terminal swallowing Cmd+C as a control sequence. Harmless on Windows/Linux
  // where there is no Cmd key.
  { actionId: "copy", binding: "Cmd+C" },
