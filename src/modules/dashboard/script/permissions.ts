...
<html><head>
  ...
  <script>
    (function () {
      ...
      // has stopped firing. Throttled to one message every ~500 ms
      // regardless of frame rate so we never spam the bridge.
      ...
      function runKkRafPump(timestamp) {
        ...
        // Heartbeat for animation-lifecycle stall detection. Throttled to
        // one post per KK_MOTION_TICK_MIN_MS so a 60 fps widget produces
        // ~2 messages/s, not 60. The parent's stall watchdog flips an
