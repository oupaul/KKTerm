# installer.status.partial

- **English value**: `Partially installed ({{installed}}/{{total}})`
- **Namespace**: `installer`
- **File/component**: `src/modules/installer/ToolRow.tsx`
- **UI role**: `status`
- **User flow**: Badge shown on a bundle tool row when some but not all of its steps are installed.
- **Tone**: short status label
- **Placeholders**: {{installed}}, {{total}}
- **Domain notes**: "Installer Helper" is the capital-M Module name on the Activity Rail; keep the proper noun phrasing recognisable. Technical terms like UAC, WSL, PATH, npm, winget, MSI typically stay English. "Update all" is the canonical button label.

<!--
Filename: installer.status.partial.md
Delete this file once every non-English locale under src/i18n/locales/ has the key translated.
-->
