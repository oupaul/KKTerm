...
The tray `app.trayWallpaper` submenu is a Windows desktop integration surface, not a Dashboard Module. `app.trayWallpaperSet` opens a small standalone `DesktopWallpaperPicker` WebView just above the tray icon without restoring or focusing the main KKTerm window. The picker reuses `SharedBackgroundPopover` so Dashboard Views, terminal Connection backgrounds, and desktop wallpaper share the same presets, media import path, fit/dim controls, and dynamic background registry. `app.trayWallpaperClear` destroys the WorkerW-hosted wallpaper WebView and persists `desktopWallpaperEnabled=false` while preserving the last selected `desktopWallpaperBackground` for the next Set action. The native host lives in `src-tauri/src/desktop_wallpaper.rs`; it creates a non-focusable frameless WebView behind desktop icons via the Windows WorkerW/Progman path, overscans the virtual screen bounds to avoid edge leakage, restores persisted wallpaper state on app startup, and emits pause state when a foreground window is maximized or covers a monitor so animated/video backgrounds can stop rendering under fullscreen work.
...
- `src/app/DesktopWallpaperPicker.tsx` — standalone tray-launched wallpaper picker route. Reuses `SharedBackgroundPopover` and persists through the desktop wallpaper Tauri commands.
