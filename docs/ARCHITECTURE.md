...
Terminal Connection backgrounds use the same shared background picker datasource as Dashboard Views: `src/modules/dashboard/edit/SharedBackgroundPopover.tsx` consumes `BACKGROUND_PRESETS`, `DYNAMIC_BACKGROUNDS`, media import/load helpers, fit options, and dim controls for both `dashboard.changeBackground` and `terminal.background`. Future background presets, dynamic backgrounds, media modes, fit options, or picker layout changes must flow through that shared datasource/component so the Dashboard and terminal lists stay identical. By default, a terminal Connection Tab paints one background behind the terminal workspace content area. The persisted Workspace setting `separateSplitTerminalBackgrounds` is off by default; when enabled and a terminal Tab has multiple terminal Panes, each Pane may store a separate `terminalBackground` in the serialized terminal layout. Missing or invalid persisted settings normalize through the Settings/storage validation boundary, clearing back to the default shared-background mode.
