...
The persisted Workspace setting `doubleClickOpensConnection` is off by default: saved Connection rows open on single click. When enabled, a saved Connection row opens only from the row double-click handler; this intentionally reserves double-click for opening and disables double-click rename for saved Connection rows, leaving the context-menu `connections.rename` action as the rename path.
