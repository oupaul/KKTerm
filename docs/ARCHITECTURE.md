...
- **Debug logs**: raw assistant and MCP debug logging is local and sensitive. Release builds write full `aiassistant.debug.log` and `mcp.debug.log` only when Settings → General → Debug → Advanced Debugging is enabled. Treat those files as user-private diagnostic data.
