...
When KKTerm itself runs inside a remote session (launched over mstsc on an RDP host), ending and resuming the mstsc connection tears down the host's display/GPU device underneath WebView2. WebView2's GPU process loses its DirectComposition device and the renderer hangs on reconnect — the native heartbeat thread keeps logging while the frontend heartbeat ages out (see `src-tauri/src/debug_heartbeat.rs`). To survive this, `configure_webview2_for_remote_session` in `src-tauri/src/lib.rs` checks `GetSystemMetrics(SM_REMOTESESSION)` at startup and, only in a remote session, appends `--disable-gpu --disable-features=CalculateNativeWinOcclusion` to `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` before the WebView2 environment is created. This forces software compositing (no GPU device to lose) and disables native-window occlusion throttling. It must run before the Tauri window is built, and it stays gated to remote sessions so local installs keep full GPU acceleration. This is a WebView2 runtime workaround and is unrelated to the RDP ActiveX parking path above.
