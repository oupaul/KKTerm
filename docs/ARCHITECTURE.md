...
- **Managed Windows service helpers** are allowlisted per tool and backed by NSSM. Installed n8n, Flowise, Open WebUI, Langflow, Excalidraw, and Ollama expose an explicit Register as service action that registers a `KKTerm-*` service with the same managed app-local command and environment, sets startup to automatic, and starts the service. Recipes that expose service registration declare `nssm` as a prerequisite so the wrapper is installed before the managed server app. The helper may trigger UAC; arbitrary catalog commands are not accepted.
