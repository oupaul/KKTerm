...
Debug builds may also create `aiassistant.debug.log` and `mcp.debug.log` beside `kkterm.log`. Release builds create full AI Assistant and MCP debug logs only when the user enables **Settings → General → Debug → Advanced Debugging**. Enabling that setting writes an `advanced_debugging.enabled` marker to both debug logs so the active release logging path is visible immediately. These files are not ordinary release telemetry; they are raw local troubleshooting logs for AI Assistant interactions and MCP traffic, including provider payloads, stream chunks, tool calls/results, permission blocks, live Session bridge traffic, MCP arguments/results, and Dashboard widget creation checkpoints. They may contain prompts, attached context, terminal buffer text returned through tools, generated widget source, and other user-provided content. Review them carefully before sharing.
