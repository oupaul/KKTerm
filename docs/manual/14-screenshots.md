...
- `screenshots.captureRegion` (tutorial target `screenshots.captureRegion`) — native full-desktop overlay; drag to select a rectangle, Esc cancels. Linux prefers the desktop screenshot portal and falls back to an installed desktop selector.
- `screenshots.captureWindow` (tutorial target `screenshots.captureWindow`) — native window picker on Windows/macOS; on Linux it captures the foreground desktop window after KKTerm minimizes.
...
The unified dialog uses the shared `Sheet`, opens at 80% of the app window, and can be resized from its bottom-right handle (`screenshots.editor.resizeDialog`). Its icon-only top toolbar contains previous/next, `screenshots.editor.undo`, `screenshots.menu.copy`, `screenshots.menu.openExternal`, `screenshots.menu.reveal`, delete, annotation tools, and zoom controls. Dimensions and file size stay centered in the bottom bar. It starts in Fit mode so oversized images remain contained; `workspace.fileViewer.zoomOut`, `workspace.fileViewer.zoomIn`, and `workspace.fileViewer.fit` provide 25–200% zoom with scrolling at explicit sizes. It can place `screenshots.editor.arrow`, `screenshots.editor.rectangle`, `screenshots.editor.ellipse`, and formatted `screenshots.editor.text` annotations or pixelate a dragged region with `screenshots.editor.mosaic`. The text mini editor controls the text, app/generic font family, size, custom color, and bold weight. Save creates a new PNG copy in the library and does not overwrite the original. Closing with edits opens `screenshots.editor.unsavedTitle`, offering Save, `screenshots.editor.dontSave`, or Cancel.
