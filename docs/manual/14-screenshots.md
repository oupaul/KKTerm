...
The unified dialog uses the shared `Sheet` and platform-aware action footer. It starts in Fit mode so oversized images remain contained; `workspace.fileViewer.zoomOut`, `workspace.fileViewer.zoomIn`, and `workspace.fileViewer.fit` provide 25–200% zoom with scrolling at explicit sizes. It can place `screenshots.editor.arrow`, `screenshots.editor.rectangle`, `screenshots.editor.ellipse`, and `screenshots.editor.text` annotations or pixelate a dragged region with `screenshots.editor.mosaic`. Undo is available while editing. Save creates a new PNG copy in the library and does not overwrite the original.
