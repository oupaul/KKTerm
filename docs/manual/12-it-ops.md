...
Open Fleets from the left navigator's add menu and choose `itops.racks.addFleet`. Enter `itops.fleets.nameLabel`, select Connections, and choose the per-Fleet transport default. `itops.transport.auto` derives transport from each Connection; explicit SSH, WinRM, and PsExec choices override it. Save with `itops.actions.create`. Editing membership does not modify or delete the referenced Connections.
