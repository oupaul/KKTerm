...
- Remote (`-R`) forwards ask the SSH server to open the configured remote listener and bridge incoming connections to the configured destination on this PC. The dialog's left fields identify the local destination, while the right fields identify the server listener. New Remote mappings default the server listener to `0.0.0.0`, which exposes it on all remote IPv4 interfaces when the SSH server's forwarding and gateway policy permits it.
