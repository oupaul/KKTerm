...
- Debug subsection `settings.debug`: toggle `settings.advancedDebugging` (hint `settings.advancedDebuggingHint`) enables full AI Assistant, MCP, Installer Helper, UI, and heartbeat debug log writing even in release builds. Enabling it writes an `advanced_debugging.enabled` marker to the JSONL logs `aiassistant.debug.log`, `mcp.debug.log`, `installer.helper.debug.log`, and `ui.debug.log` beside `kkterm.log`, so the release logging path is visible before the next assistant, MCP, Installer Helper, or UI event. It also starts `kkterm-heartbeat.debug.log`; turning the setting off stops release heartbeat writes. The `ui.debug.log` records frontend UI diagnostics such as terminal focus-restore attempts (document focus state and the active element) used to troubleshoot input focus after switching apps. These local logs may include prompts, tool arguments, MCP arguments/results, Installer Helper command output, local paths, frontend/native liveness timing, screenshots, and generated Dashboard AI Created Widget source.
