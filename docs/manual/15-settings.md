...
- Settings data subsection (destructive actions live here):
  ...
  - Export Selection: `settings.selectiveExport` opens a dialog with a switch per category (`settings.segment_connections`, `settings.segment_workspaces`, `settings.segment_dashboards`, `settings.segment_settings`, `settings.segment_mcpServers`) and an off-by-default `settings.includeCredentials` switch that, when enabled, requires a passphrase (`settings.exportPassphrase`) and warns via `settings.includeCredentialsWarning`. Produces a `.kkbackup` file; success `settings.selectiveExportComplete`.
  - Import Selection: `settings.selectiveImport` inspects the chosen `.kkbackup` and shows a per-category action selector — `settings.importActionSkip` / `settings.importActionAdd` / `settings.importActionReplace` — plus a passphrase field for encrypted bundles. A safety backup runs first (`settings.selectiveImportWarning`); success `settings.selectiveImportComplete`. See [17-data-backup-secrets.md](17-data-backup-secrets.md).
