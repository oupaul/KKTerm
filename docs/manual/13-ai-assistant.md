...
`aiassistant.debug.log` is a local troubleshooting log for AI Assistant interactions. Debug builds write it automatically; release builds write full AI Assistant debug logs only when Settings → General → Debug → `settings.advancedDebugging` is enabled. The log may include raw prompts, attached context, screenshots/data URLs, tool calls/results, and generated Dashboard widget source. Debug builds also write raw built-in and remote MCP request/response records to `mcp.debug.log`; release builds write the same MCP log when `settings.advancedDebugging` is enabled. The same setting enables `installer.helper.debug.log` for Installer Helper operations, `url.connection.debug.log` for URL WebView2 overlay geometry, `rdp.debug.log` for RDP startup/display diagnostics, and `kkterm-heartbeat.debug.log` for frontend/native liveness timing in release builds. Turning on Advanced Debugging writes an `advanced_debugging.enabled` marker to the JSONL debug logs so users can verify the option is active before the next assistant, MCP, Installer Helper, UI, URL Connection, or RDP event. Users should review these files before sharing.
