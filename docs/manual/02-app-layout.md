...
2. **Transient notifications** — driven by the shared `showStatusBarNotice` store action. Messages render as Pulse-style popups above the Status Bar, with direct tone icons for success, info, warning, and error. Success/info/warning messages default to 5 seconds with a running progress hairline; errors stay until dismissed. Determinate work uses the shared `showStatusBarProgress` variant with a `CircleGauge` icon, a real 0–100% bar, and fixed endpoint labels instead of the five-second timer. App update downloads use this variant; Windows downloads can be cancelled with `settings.updateDownloadCancel`, while completed downloads remain at 100% for three seconds before installation. Do not implement one-off toast surfaces; route through the shared Status Bar notice actions.
