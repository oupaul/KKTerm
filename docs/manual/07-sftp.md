...
- **Text** (`compare.mode.text`) — a side-by-side line diff with the same search, all/diff/same filter, change navigation, and minimap as the Git Browser's advanced diff. Modified line pairs get Beyond Compare-style **per-character highlighting**: a character-level diff (shared prefix/suffix trim plus an LCS over the differing middle) paints the unchanged runs on the line's tint and the changed runs in a stronger block (`.seg-diff`), so you see exactly which characters changed.
