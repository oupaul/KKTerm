...
An installed Windows copy can also create a launch-ready portable folder from **Settings → General → Settings data** using `settings.portableCreatorAction`. The wizard copies the current executable and bundled resources, then creates a portable database from the selected non-secret categories. The destination must be an empty writable local/removable folder; saved passwords are not copied.
