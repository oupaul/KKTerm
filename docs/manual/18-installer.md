...
- **Installed info** — version, install location (when known), provider summary, latest version + last checked timestamp, pin-version checkbox, and an Update banner when `latestVersionSeen` differs from `installedVersion`. Installed tools that expose a local browser interface may also show `installer.dialog.webUi` plus `installer.actions.run` and `installer.actions.openWebUi`; for n8n, `installer.actions.run` starts `n8n start`, and `installer.actions.openWebUi` opens `http://localhost:5678` in the external browser. Footer: `Uninstall` (danger) · `Run`/`Open web UI` (when available) · `Update` (when available) · `Close`.
