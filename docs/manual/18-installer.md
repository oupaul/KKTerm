...
Each tool surface is a **tile** in the section grid. Installed tools show an `installer.section.installed` badge. Tile version metadata is shown on separate `installer.tile.latest` and installed-version rows, rather than as provider kind text. Runtime bundles use `installer.tile.nodeVersion` and `installer.tile.pythonVersion` for their installed-version label after detection. The page-level header status carries the most recent check time. One-step bundles usually inherit latest/detected version metadata from their child recipe; Node and Python bundles instead show the detected runtime version once nvm-windows/uv and the managed runtime are present. The tile action button switches between `installer.actions.install` and `installer.actions.uninstall` based on detected install state, and opens the app-owned popup dialog — `InstallerToolDialog` — that owns the detail surface (the previous inline expansion has been removed). Tutorial target on the options form inside the dialog: `installer.toolOptions`.
