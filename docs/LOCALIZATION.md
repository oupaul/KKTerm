...
- Appears in: `src/settings/GeneralSettings.tsx`
- UI role: Button label
- Context: Button in General Settings that warns the user, opens a file picker for an AdminDeck export zip, and replaces the current database with the imported database.
- Tone: Concise command label
- Placeholders: None
...
- Tone: Brief confirmation
- Placeholders: None
- Domain notes: Import replaces the current local SQLite database with the imported AdminDeck export.
- Translation status: Pending for fr, it, de, es, es-MX, pt-BR, zh-TW, zh-CN, ja, ko, th, id
