...
| settings.minimizeToTray | Minimize to tray | settings | src/settings/GeneralSettings.tsx | toggle label | Enables hiding the desktop app in the system tray when minimized | short label | None | Tray is the OS notification/system tray, not an AdminDeck Tab |
| settings.minimizeToTrayHint | Hides AdminDeck in the system tray when the window is minimized. | settings | src/settings/GeneralSettings.tsx | hint | Explains minimize-to-tray behavior in General settings | concise, practical | None | Close still uses the native app close path |
