...
- Settings: the current surface lives in `src/settings/SettingsPage.tsx` and is ordered as General, Appearance, AI Assistant, SSH, Terminal, Remote Desktop(RDP), VNC, and About. General contains Language (i18n) as a planned placeholder; Appearance contains App UI font, layout reset, and Color Scheme as a planned placeholder; SSH includes SSH defaults and SFTP transfer defaults; Terminal contains editable terminal behavior; RDP and VNC expose planned quality default summaries. Diagnostics, editable SSH/SFTP defaults, update, SSH config import, and keybinding controls should be reintroduced only when their UX is clear and backed by the existing local storage/keychain boundaries.
