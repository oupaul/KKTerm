...
Pick a mood — per dashboard view, *or behind any terminal* — from canvas-animated backgrounds:
...
The same picker backs your terminal panes too, so you can drop `matrix` behind a live SSH session. They pause when you're elsewhere, so they cost roughly nothing.
