...
<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — 整個工作區一眼看完：左邊連線樹、中間一格格 live Pane、右邊 AI 助理。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — 一個 Tab，多種 Connection 類型同居：SSH 挨著 SFTP 挨著內嵌 Web UI。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — 四格網格：PowerShell、一個 SSH session、一個 SFTP 瀏覽器、一個 VNC 畫面，全在同一個 Tab。</em></sub></p>
...
它還能看見你看見的：用**截圖選單**框一塊區域或整個 Pane，直接送進對話，這樣「為什麼這個對話框長得怪怪的？」就變成助理真的答得出來的問題。

<p align="center">
  <img src="docs/assets/screenshots/ai-assistant.png" alt="AI 助理面板，含工具存取與審批模式開關" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — AI 助理面板：整類工具開關、Prompt / Allow All 切換，以及一個正在等人類點頭的危險指令。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — 一個塞滿 AI 做的 widget 的 Dashboard view：ping 監控、便利貼、即時數字，還有一個好玩到不像話的小玩具。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — 各種心情的拼貼：`matrix`、`aurora`、`synthwave`、`taipei101`。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — Pane 工具列上的 tmux/psmux session 清單，重連後遠端的 `claude` agent 仍在跑。</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/usage-meter.png" alt="AI 用量計 widget 與狀態列指示器" width="720" />
</p>
...
<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — Activity Rail 頂端的工作區切換器，正在「Home Lab」和「Day Job」之間切換。</em></sub></p>

### 檔案和 log，都在同一扇視窗

不是每樣東西都是遠端主機。KKTerm 能用 File Explorer Pane 瀏覽你的**本機磁碟**（和 SFTP 同一套雙窗格外殼），也能用 **Document 檢視器**打開單一檔案，並自動挑對模式：帶輕量編輯與安全儲存的文字／程式碼、Markdown、CSV/TSV 表格、JSON、圖片、PDF，還有專門的 **Log 模式**，支援等級上色、篩選、ANSI 與 tail 跟隨。再也不用為了讀一份你就站在旁邊的 log，特地去開另一個編輯器。

<p align="center">
  <img src="docs/assets/screenshots/file-viewer.png" alt="Document 檢視器以 log tail 模式顯示，旁邊是 File Explorer pane" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — Document 檢視器跟隨一份即時 log（等級上色＋篩選），旁邊是本機 File Explorer Pane。</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/mcp-server.png" alt="內建 MCP 伺服器設定與顯示設定對話框" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>截圖佔位</strong> — Built-in MCP Server 面板，含「顯示設定」對話框與預設關閉的安全開關。</em></sub></p>
