...
- **有審批邊界的 AI**：聊天、選取的終端 context、截圖、指令提案、Dashboard 工具、已儲存 Connection 管理工具、即時 Session 互動工具，以及多 provider 設定；以 Prompt 模式或 Allow All 模式控制是否自動執行。
...
| **AI Assistant** | 串流聊天、OpenAI-compatible runtime、provider registry、指令提案安全分類、截圖/context 附件、Dashboard tool calls、已儲存 Connection 管理工具，以及終端、RDP/VNC 與 SFTP/FTP 互動的即時 Session 工具 |
...
KKTerm 持續快速發布。這份 README 描述目前 codebase 的實際狀態。

- Windows 是主要支援平台；macOS 與 Linux 打包列於規劃中。
- Installer 目前尚未簽章。Release signing 設定完成前，update checks 暫停。
- 原生 SFTP 路徑尚不支援 ProxyJump。
- File transfer resume、folder sync/diff、archive/extract 與 remote editing 尚未實作。
- SSH config import 已在後端實作，但 Settings 尚未對外暴露入口。
- RDP 與 VNC 已正式出貨；更完整的 clipboard/device sync 與進階品質控制仍在演進。
- AI 可以輔助、提案並在 Prompt/Allow All 邊界內操作已啟用的工具，但不應被視為無人值守的自主 operator。
