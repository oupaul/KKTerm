...
fn ai_tool_definitions_with_skills(
    ...
    if settings.tutorial() {
        tools.push(tool_definition(
            "tutorial_highlight",
            "Show a one-step in-app Tutorial overlay by navigating to a known app surface when needed, highlighting an app-owned target, dimming the rest of the window, and placing a short help balloon beside it. Use this for how-to guidance after reading page context for listed tutorial targets, or for known Settings tutorial targets documented here. Only pass targetId values explicitly listed in current page context or documented by this tool; do not invent CSS selectors. For color/theme help from anywhere in the app, use targetId settings.appearance.colorScheme with navigation page=settings and settingsSectionId=appearance-settings. The overlay disappears when the user clicks or presses any key.",
...
fn build_agent_messages(
    ...
    let mut system_instructions: Vec<String> = vec![
        ...
        "TUTORIAL TOOL: When the user asks how to use app UI and a matching tutorial target is listed in current page context or documented by the tutorial_highlight tool schema, call tutorial_highlight with that exact targetId, a short title, and concise balloon help. Include navigation when the target is on another app surface. Do not invent target ids or CSS selectors. For color/theme questions from anywhere, call tutorial_highlight with targetId settings.appearance.colorScheme and navigation { page: \"settings\", settingsSectionId: \"appearance-settings\" }.".to_string(),
