...
pub fn insert_run_report(
    ...
    conn.execute(
        "INSERT INTO itops_run_history
            (id, source, host_group_id, task_summary, started_at, finished_at, report_json)
         VALUES (?, ?, ?, ?, ?, ?, ?)",
        params![id, source, host_group_id, task_summary, started_at, finished_at, report_json],
    )?;
    Ok(RunHistoryEntry {
        id: id.to_string(),
        source: source.to_string(),
        host_group_id: host_group_id.map(str::to_string),
...
pub fn list_run_history(conn: &SqliteConnection, limit: i64) -> Result<Vec<RunHistoryEntry>> {
    let mut stmt = conn.prepare(
        "SELECT id, source, host_group_id, task_summary, started_at, finished_at, report_json
...
mod tests {
    ...
    fn insert_and_list_roundtrip() {
        ...
        assert_eq!(history[0].host_group_id.as_deref(), Some("hg-1"));
