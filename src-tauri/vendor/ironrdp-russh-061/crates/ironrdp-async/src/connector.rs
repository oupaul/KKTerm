...
where
    ...
    N: NetworkClient,
{
    connect_finalize_with_channel_bindings(
        Upgraded,
        connector,
        framed,
        network_client,
...
/// Same as [`connect_finalize`], but additionally accepts an RFC 5929
/// `tls-server-end-point` channel binding token (`b"tls-server-end-point:"`
/// followed by the TLS server certificate hash) to apply to the inner NTLM
/// context, so servers enforcing Extended Protection for Authentication (EPA)
/// accept the logon. Ignored when Kerberos is used instead of NTLM
/// (`kerberos_config` is `Some`).
pub async fn connect_finalize_with_channel_bindings<S, N>(
    _: Upgraded,
