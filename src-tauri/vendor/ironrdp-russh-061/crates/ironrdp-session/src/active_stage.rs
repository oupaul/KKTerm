...
/// Diagnostic hook for tracking down tile-position/ghosting rendering bugs
/// against legacy hosts that lean heavily on slow-path (X224) bitmap updates.
/// Set once by the embedding application via [`set_slow_path_bitmap_diagnostic_hook`];
/// called once per `TS_BITMAP_DATA` rectangle inside a slow-path Bitmap Update
/// PDU, before it is handed to the shared bitmap-apply pipeline. Args: left,
/// top, right, bottom (as encoded on the wire), width, height, bits_per_pixel,
/// compressed, bitmap_data length in bytes.
pub static SLOW_PATH_BITMAP_DIAGNOSTIC_HOOK: std::sync::OnceLock<
    fn(u16, u16, u16, u16, u16, u16, u16, bool, usize),
> = std::sync::OnceLock::new();

pub fn set_slow_path_bitmap_diagnostic_hook(hook: fn(u16, u16, u16, u16, u16, u16, u16, bool, usize)) {
...
fn process_slow_path_graphics(
    ...
    match update_type {
        GraphicsUpdateType::Bitmap => {
            ...
            if let Some(hook) = SLOW_PATH_BITMAP_DIAGNOSTIC_HOOK.get() {
                for rect in &bitmap.rectangles {
                    hook(
                        ...
                        rect.height,
                        rect.bits_per_pixel,
                        rect.compression_flags.contains(ironrdp_pdu::bitmap::Compression::BITMAP_COMPRESSION),
