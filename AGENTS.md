...
End-user operation docs live in `docs/manual/` and ship with the app. `docs/manual/INDEX.md` is the entry point; chapters cover the rail Modules (Workspace, Dashboard, File Explorer, Settings) plus their sub-features (Connections, Terminal, SSH/tmux, SFTP, URL, RDP/VNC, App Launcher, Wiki, AI Assistant, Screenshots, Localization, Data/Backup). The manual references i18n keys (e.g. `connections.quickConnect`), not English labels, so locale changes do not invalidate it.

**Update rule:** any PR that changes UI behavior in a chapter's scope must update that chapter in the same PR. When a key is renamed or removed, run `git grep "<old.key>" docs/manual` and fix every reference. New UI surfaces require either a new chapter or a new section in the closest existing chapter — choose by the owning Module or by topical proximity, matching `INDEX.md`. Do not document English label text directly; reference the i18n key. The in-app AI Assistant grep-searches `docs/manual/` to answer "how do I…" questions, so keep each chapter's `## AI grep hints` block accurate (keys, file paths, common synonyms).
...
or single Rust source file. Do not run broad `cargo fmt` over the whole workspace unless
...
- **Module** (capital M): a fixed, top-level pillar of the app that lives in the Activity Rail. Modules in this build are **Workspace**, **Dashboard**, and **File Explorer**, with **Settings** as the bottom rail destination. Each Module owns its own page region, its own i18n namespace where applicable, and (for MCP) its own `kkterm.<module>.*` tool namespace. The Activity Rail also shows connected Connection shortcuts, but those are *not* Modules. The App Launcher is *not* a Module — it is a Dashboard widget. Reserve the capitalised term for these rail destinations; do not call source directories, Rust `mod` items, CSS bundles, or sub-features "Modules". Use "source area", "Rust source file", "feature CSS", "section", or "area" instead.
...
- User-facing transient status messages belong in the bottom workspace status bar. Use the shared `showWorkspaceStatus` store action, keep success confirmations at the default 5 second duration unless the product flow explicitly needs otherwise, and let `src/modules/workspace/StatusBar.tsx` own fade-in/fade-out behavior. Do not add one-off toast/status implementations for Connection lifecycle events.
...
- **Source area structure.** Frontend source is organized so the directory tree mirrors the Module language from `CONTEXT.md`:
  - `src/modules/<module>/` is the home of every rail Module (`workspace`, `dashboard`, `settings`). All Module-owned UI and helpers live under here.
  - `src/modules/workspace/connections/<kind>/` is the home of every per-Connection-kind workspace surface (`terminal`, `webview`, `sftp`, `remote-desktop`). The connection-tree code lives one level up at `src/modules/workspace/connections/`. The `remote-desktop/` folder owns **both** RDP and VNC — they share `RemoteDesktopWorkspace.tsx` and split on `connection.type` internally. Do not create a separate `vnc/` folder.
  - `src/modules/dashboard/widgets/builtin/<widget>/` is the home of every built-in Dashboard widget implementation (`app-launcher`, `ai-coding-usage`, `connections`, `notes`). Each widget owns its component, CSS, storage, and tests. The dispatcher stub in `src/modules/dashboard/widgets/<WidgetName>Body.tsx` re-exports from `builtin/<widget>/` so `registry/builtInRegistry.ts` can keep importing by stable name.
  - `src/app/` is the shell that **hosts** Modules (Activity Rail, TitleBar, TutorialOverlay, workspace chrome, app-shell effects). It is **not** a Module.
  - `src/ai/` is the AI Assistant Panel UI — panel UI, **not** a Module.
  - `src/manual/`, `src/i18n/`, `src/lib/`, `src/styles/`, `src/assets/` are cross-cutting roots and stay at `src/` root.
  - `src/App.tsx`: app shell routing, Settings routing, bootstrap hook composition, and workspace chrome composition only — it does not own form/control code, feature surfaces, Activity Rail internals, or panel resize mechanics.
  ...
  **Forward-placement rule.** New rail-Module UI goes under `src/modules/<module>/`. New per-Connection-kind workspace surfaces go under `src/modules/workspace/connections/<kind>/`. New built-in Dashboard widgets go under `src/modules/dashboard/widgets/builtin/<widget>/` with a thin dispatcher stub in `src/modules/dashboard/widgets/`. Cross-cutting helpers stay at `src/` root. The Workspace `StatusBar` rendering a Dashboard widget's compact status presenter (e.g. `AiCodingUsageStatusBar`) is the one allowed Module-to-Module read; route any new such read through a similarly minimal import and document it here.
...
- AI provider model choices are owned by `src/ai/providerRegistry/` and rendered in `src/modules/settings/AiSettings.tsx`. Keep the known model picker as a real provider-specific `<select>` so all options are visible; keep exact/custom model IDs in the separate custom model input. Do not use an `<input list>`/`datalist` for provider model selection because Chromium filters it to the current value and hides the rest of the provider list.
- See `docs/ARCHITECTURE.md` "Frontend Source Map" before placing new UI or helper logic.
- Native HWND-backed RDP ActiveX cannot be trusted to obey DOM z-index, so only RDP uses overlay parking. Preserve the RDP screenshot/parking behavior in `src/modules/workspace/connections/remote-desktop/RemoteDesktopWorkspace.tsx`: when an app-owned DOM overlay intersects the RDP host rectangle, capture the visible RDP host via the typed screenshot command, show that bitmap underneath the DOM overlay, then hide/park the ActiveX HWND until the overlay closes. Do not apply overlay parking or the RDP screenshot workaround to WebView2, terminal, SFTP, or VNC workspaces. WebView2 is moved/hidden only for URL Session lifecycle needs such as inactive Tabs, not for app menus. Keep RDP overlay detection centralized and geometry-scoped in `src/modules/workspace/nativeOverlay.ts`.
...
- Terminal and WebView2 surfaces can use normal app stacking for popovers and non-native overlays. If a new DOM overlay must cross into an active RDP workspace, verify it over active terminal/RDP/WebView2 sessions and update `src/modules/workspace/nativeOverlay.ts` only for the RDP ActiveX case.
...
- `withLiveConnectionStatuses` in `src/modules/workspace/connections/treeUtils.ts` spreads every connection (`{...connection, status}`) and therefore returns a fresh `Connection` reference on every `activeSessionCounts` change. Use it only on display surfaces (status dots, connection-tree badges, the dashboard Connection widget tab strip). Do not hand a live-status `Connection` to `TerminalWorkspace`, `WebViewWorkspace`, `RemoteDesktopWorkspace`, or `SftpWorkspace` — those workspaces own session lifecycle effects (most notably the PTY-managing `useEffect` in `src/modules/workspace/connections/terminal/TerminalWorkspace.tsx` which lists `pane.connection` in its dependency array) and a reference change tears down and restarts the session, which itself bumps `activeSessionCounts` and produces an unbounded mount/unmount flicker loop. When embedding a workspace component (Connection widget, future dashboard/popover embeds), look the active `Connection` up by `id` from the raw `tree` (or any non-augmented source) so the reference stays stable across status changes; see `src/modules/dashboard/widgets/ConnectionWidgetBody.tsx` for the pattern (`sessionConnectionsById`).
