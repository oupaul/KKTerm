...
- Terminal and WebView2 surfaces can use normal app stacking for menus and popovers. Keep Quick Connect and other sidebar flyout menus geometrically inside the sidebar when possible; if a flyout must cross into the workspace, verify it over active terminal/RDP/WebView2 sessions and update CSS/layout for DOM/WebView2 issues, not overlay parking.
