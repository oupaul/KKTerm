...
- **IT Ops モジュール**。スクリプトや対話型のplaybookを、ホストのグループまるごとに一斉展開できる。さらに、pingして・見張って・何かおかしくなったらメールしてくれる **watchdog 自動化** も。
...
<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — ワークスペース全体をひと目で：左に接続ツリー、中央にライブPaneのグリッド、右にAIアシスタント。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — 1つのTabに複数のConnectionタイプが同居：SSHの隣にSFTP、その隣に組み込みWeb UI。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — 4分割グリッド：PowerShell、SSHセッション、SFTPブラウザ、VNC画面が、すべて1つのTabの中に。</em></sub></p>
...
あなたが見ているものを、AIも見られる：**スクリーンショットメニュー**で領域やPane全体をつかんで、そのまま会話に送り込める。だから「このダイアログ、なんで変な見た目なの？」が、アシスタントが実際に答えられる質問になる。

<p align="center">
  <img src="docs/assets/screenshots/ai-assistant.png" alt="ツールアクセスと承認モードのトグルを備えたAIアシスタントパネル" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — AIアシスタントパネル：ツールカテゴリのトグル、Prompt / Allow All の切り替え、そして人間のイエスを待つ危険なコマンド。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — AIが作ったWidgetで埋まったDashboard View：ping監視、付箋、ライブの数字、そしてこんなに楽しくていいのかという小さなおもちゃ。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — 気分の見本：`matrix`、`aurora`、`synthwave`、`taipei101`。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — Paneツールバーのtmux/psmuxセッション一覧。再接続後もリモートの `claude` エージェントは動き続けている。</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/usage-meter.png" alt="AI使用量メーターのWidgetとステータスバーインジケーター" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — Claude Code と Codex を並べて表示する使用量Widgetと、コンパクトなステータスバーのミラー。</em></sub></p>

### ウィンドウを離れずにフリート運用

「1台のサーバー」が「40台のサーバー」になると、シングルペインのワークフローはスケールしなくなる。**IT Ops モジュール**は、KKTerm が大人になる場所だ：

- **Host Groups（ホストグループ）** — Connection の名前付きの選択（任意で動的フィルター付き）。以下のすべてのターゲットとして再利用する。
- **Batch Runs（一括実行）** — 一回限りのスクリプト*または*対話型のexpect形式の **Playbook** を、SSH・WinRM・PsExec 経由でホストグループに一斉展開する。ホストごとのライブ進捗と、再度開ける集約レポート付き。
- **Automations と Watchdog** — ルール（ping、TCP到達性、パフォーマンスカウンター、あるいはSSHセッションが静かになったこと）を仕掛けて、発火したときに何をするかを選ぶ：通知、ポップアップ、メール、webhook、Batch Run の起動、あるいはAIに委ねる。ルール本体は再起動を生き延びる；ライブの Watchdog はステータスバーに現れる。

NOC の代替ではない。「昼までにこの12台にパッチを当てないと」という、ターミナルにずっと欲しかったツールだ。

<p align="center">
  <img src="docs/assets/screenshots/it-ops.png" alt="ホストグループに対して一括実行を走らせるIT Opsモジュール" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — ホストごとの出力をライブでストリーミングする Batch Run のグリッドと、その隣に仕掛け済みの Watchdog 自動化。</em></sub></p>
...
<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — Activity Rail の上部にあるワークスペース切り替え。「Home Lab」と「Day Job」を切り替えている途中。</em></sub></p>

### ファイルとログも、同じウィンドウの中で

すべてがリモートホストとは限らない。KKTerm は File Explorer Pane で**ローカルディスク**を閲覧でき（SFTPと同じデュアルペインの外殻）、**Documentビューア**で1つのファイルを開いて、用途に合ったモードを自動で選ぶ：軽量エディターと安全保存付きのテキスト/コード、Markdown、CSV/TSVテーブル、JSON、画像、PDF、そしてレベル色分け・フィルター・ANSI・tail追従を備えた専用の **Log モード**。すぐ隣にあるログを読むためだけに、別のエディターに手を伸ばす必要はもうない。

<p align="center">
  <img src="docs/assets/screenshots/file-viewer.png" alt="File Explorer ペインの隣で log tail モードのDocumentビューア" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — ライブのログを追従する Document ビューア（レベル色分け＋フィルター）と、その隣のローカル File Explorer Pane。</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/mcp-server.png" alt="組み込みMCPサーバーの設定と設定表示ダイアログ" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>スクリーンショット（仮）</strong> — Built-in MCP Server パネル。「設定を表示」ダイアログと、既定でオフの安全トグル。</em></sub></p>
