...
- 一个 **IT Ops 模块**，能把一段脚本或交互式 playbook 一次撒到一整群主机上，再加上会 ping、会盯、出事就 email 你的 **watchdog 自动化**。
...
它还能看见你看见的：用**截图菜单**框一块区域或整个 Pane，直接送进对话，这样「为什么这个对话框长得怪怪的？」就变成助理真的答得出来的问题。
...
<p align="center">
  <img src="docs/assets/screenshots/usage-meter.png" alt="AI 用量计 widget 与状态栏指示器" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>截图占位</strong> — 用量 widget 把 Claude Code 和 Codex 并排显示，再加上精简的状态栏镜像。</em></sub></p>

### 不用离开窗口就能管整批主机

当「一台服务器」变成「四十台服务器」，单窗格的工作流就撑不住了。**IT Ops 模块**就是 KKTerm 长大的地方：

- **Host Groups（主机组）** — 一份命名的 Connection 选集（可加上动态筛选），下面所有功能都拿它当目标。
- **Batch Runs（批量执行）** — 把一段一次性脚本*或*一份交互式、expect 风格的 **Playbook**，通过 SSH、WinRM 或 PsExec 撒到一个主机组上，附实时的每台主机进度，以及一份可重新打开的汇总报告。
- **Automations 与 Watchdog** — 设一条规则（ping、TCP 可达性、性能计数器，或某个 SSH session 安静下来），再决定它触发时要做什么：通知、弹窗、email、webhook、启动一个 Batch Run，或交给 AI 处理。规则本身扛得过重启；live 的 Watchdog 会出现在状态栏。

它不是 NOC 的替代品。它是那个「午餐前我得把这十二台机器更新完」的工具，你的终端一直缺的就是它。

<p align="center">
  <img src="docs/assets/screenshots/it-ops.png" alt="IT Ops 模块正在对一个主机组跑批量执行" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>截图占位</strong> — Batch Run 网格实时串流每台主机的输出，旁边面板是一条已武装的 Watchdog 自动化。</em></sub></p>
...
不是每样东西都是远程主机。KKTerm 能用 File Explorer Pane 浏览你的**本机磁盘**（和 SFTP 同一套双窗格外壳），也能用 **Document 查看器**打开单个文件，并自动挑对模式：带轻量编辑与安全保存的文本／代码、Markdown、CSV/TSV 表格、JSON、图片、PDF，还有专门的 **Log 模式**，支持等级上色、筛选、ANSI 与 tail 跟随。再也不用为了读一份你就站在旁边的 log，特地去开另一个编辑器。

<p align="center">
  <img src="docs/assets/screenshots/file-viewer.png" alt="Document 查看器以 log tail 模式显示，旁边是 File Explorer pane" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>截图占位</strong> — Document 查看器跟随一份实时 log（等级上色＋筛选），旁边是本机 File Explorer Pane。</em></sub></p>
...
<p align="center">
  <img src="docs/assets/screenshots/mcp-server.png" alt="内建 MCP 服务器设置与显示配置对话框" width="720" />
</p>

<p align="center"><sub><em>📸 <strong>截图占位</strong> — Built-in MCP Server 面板，含「显示配置」对话框与默认关闭的安全开关。</em></sub></p>
