...
test("SFTP native context menu exposes cut, copy, and paste file actions", () => {
  ...
  assert.ok(workspaceSource.includes("showNativeContextMenu"));
...
test("SFTP workspace wires internal and native file clipboard operations", () => {
  ...
  assert.match(workspaceSource, /action:\s*\(\) => handleContextCut\(menu\)/);
  assert.match(workspaceSource, /action:\s*\(\) => handleContextCopy\(menu\)/);
  assert.match(workspaceSource, /action:\s*\(\) => handleContextPaste\(menu\)/);
});
