...
test("File Explorer renders the shared file browser as a single local pane", async () => {
  const [workspaceSource, manualSource] = await Promise.all([
    readFile(
  ...
  assert.match(
    workspaceSource,
    /if \(!isLocalFilesBrowser\) \{[\s\S]*t\("sftp\.download"\)[\s\S]*t\("sftp\.upload"\)/,
    "File Explorer should hide transfer commands from the native context menu",
  );
  assert.match(
    workspaceSource,
